                        ...
                        if (self.board[x.position[0] + y[0]][x.position[1] + y[1]] == NOBODY):
                            ...
                            for z in toAppend.white_pieces:
                                ...
                                if z.position == x.position:
                                    toAppend.move(
                                        z, z.position[0] + y[0], z.position[1] + y[1])
                    ...
                    if (self.board[int(x.position[0] + (z[0]/2))][int(x.position[1] + (z[1]/2))] == BLACK) and (self.board[int(x.position[0] + (z[0]))][int(x.position[1] + (z[1]))] == NOBODY):
                        ...
                        for y in toAppend.white_pieces:
                            ...
                            if y.position == x.position:
                                toAppend.move(
                                    y, y.position[0] + z[0], y.position[1] + z[1])
                                toAppend.turn = not self.turn
                                break
...
def play_against_MCTS(board):  # moves take a while, but is functional
    board = Board()
    print(board.board)
    while board.game_over == False:
        moves = board.getMoves()
...
board = Board() #initializes the gameboard, uncomment any of the below functions to run

# MCTS_play_minimax(board) #MCTS algorithm vs Minimax algorithm
# play_against_MCTS(board) #Play against the MCTS algorithm
# play_minimax(board) #Minimax algorithm play itself
